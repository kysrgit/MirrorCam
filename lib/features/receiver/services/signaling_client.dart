...
class SignalingClient {
  ...
  Future<void> _connect() async {
    try {
      ...
      Logger.debug('[DEBUG-SC] WebSocket CONNECTED to $uri');

      // Mesajları dinle
      _subscription = _channel!.stream.listen(
        (dynamic data) {
          Logger.debug('[DEBUG-SC] RAW data received: ${data.runtimeType} => $data');
          if (data is String) {
            try {
              final json = jsonDecode(data) as Map<String, dynamic>;
              Logger.debug('[DEBUG-SC] Parsed message type: ${json['type']}');
